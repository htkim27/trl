...
class ORPOTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        print("이즈 인코더 디코더 : ", is_encoder_decoder)
    ...
    def concatenated_forward(
        ...
        print(f"배치 : {concatenated_batch}")
        print(f"배치 shape : {concatenated_batch['concatenated_input_ids'].shape} / '{concatenated_batch['concatenated_labels'].shape}'")
        ...
        concatenated_input_ids = torch.split(concatenated_batch["concatenated_input_ids"], split_size_or_sections=1, dim=0)
        concatenated_attention_mask = torch.split(concatenated_batch["concatenated_attention_mask"], split_size_or_sections=1, dim=0)
        print(f"슬라이싱 셰입 {concatenated_input_ids[0].shape}")
        outputs_0 = model(
            concatenated_input_ids[0],
            attention_mask=concatenated_attention_mask[0],
            use_cache=False,
            **model_kwargs,
        )
        print("outputs 구조 : ", outputs_0)
        outputs_1 = model(
            concatenated_input_ids[1],
            attention_mask=concatenated_attention_mask[1],
            use_cache=False,
            **model_kwargs,
        )

        print(f"아웃풋_0 shape : {outputs_0.logits.shape}")
        print(f"아웃풋_1 shape : {outputs_1.logits.shape}")
        gpu = GPUtil.getGPUs()[0]

        print(f"\tFree Memory: {gpu.memoryFree}MB")
        print(f"\tUsed Memory: {gpu.memoryUsed}MB")
        print(f"\tTotal Memory: {gpu.memoryTotal}MB")
        all_logits = torch.cat((outputs_0.logits, outputs_1.logits), dim=0)
