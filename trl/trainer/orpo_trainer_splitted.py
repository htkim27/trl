...
class ORPOTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        """list the chosen and rejected inputs into a single tensor.
        ...
        for k in batch:
            if k.startswith("rejected") and isinstance(batch[k], torch.Tensor):
                ...
                concatenated_batch[concatenated_key].append(pad_to_length(batch[k], max_length, pad_value=pad_value).to(device=device))
    ...
    def concatenated_forward(
        ...
        model_kwargs = (
            {
                "decoder_input_ids": [self._shift_right(concatenated_batch["concatenated_labels"][0]),
                                      self._shift_right(concatenated_batch["concatenated_labels"][1])],
            }
            if self.is_encoder_decoder
            else {}
        )
        ...
        all_logits = [chosen_outputs.logits, rejected_outputs.logits]
        ...
        if self.is_encoder_decoder:
            labels = [label.clone() for label in concatenated_batch["concatenated_labels"]]
        else:
            labels = [label.clone() for label in concatenated_batch["concatenated_input_ids"]]

        chosen_nll_loss = cross_entropy_loss(all_logits[0], labels[0])

        chosen_logps = self.get_batch_logps(
            all_logits[0],
            concatenated_batch["concatenated_labels"][0],
